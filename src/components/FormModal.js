...
function FormModal( props ) {
  ...
  const onSubmit = data => {
    ...
    return (
        ...
        >
          <ModalHeader>Creation de rendez-vous</ModalHeader>
          <form onSubmit={handleSubmit(onSubmit)}>
          <ModalBody>
            
            <InputContainer>
              <InputSpan>Nom & prénom:</InputSpan>
              <InputField {...register("Name")}></InputField>
            </InputContainer>
            <InputContainer>
              <InputSpan>Sexe:</InputSpan>
              <InputField {...register("Gender")}></InputField>
            </InputContainer>
            <InputContainer>
              <InputSpan>Âge:</InputSpan>
            ...
            <InputContainer>
              <InputSpan>Temps:</InputSpan>
              <InputField {...register("Time")} type="number" min="1" max="12" placeholder="Type a number 1 - 12"></InputField>
            </InputContainer>
          </ModalBody>
          <ModalFooter>
              <ModalSubmit>Valider</ModalSubmit>
              <ModalCancel onClick={props.openModal}>Annuler</ModalCancel>
          </ModalFooter>
          </form>
        </Modal>
    );
